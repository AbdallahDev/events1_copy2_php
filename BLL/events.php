...
class events extends my_db {
    ...
    function insert_event($committee_id, $event_entity_name, $time, $hall_id, $event_place, $subject, $event_date, $event_status, $directorate_id, $user_id) {
        $this->mod_data('insert into events(committee_id, event_entity_name, time, hall_id, event_place, subject, event_date, event_status, directorate_id, user_id_insert) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', 'ississsiii', array(&$committee_id, &$event_entity_name, &$time, &$hall_id, &$event_place, &$subject, &$event_date, &$event_status, &$directorate_id, &$user_id));
    }

    function update_event($event_entity, $event_entity_name, $time, $hall_id, $event_place, $subject, $event_date, $event_status, $event_edit_date, $directorate_id, $user_id_edit, $id) {
        $this->mod_data('update events set committee_id = ?, event_entity_name = ?, time = ?, hall_id = ?, event_place = ?, subject = ?, event_date = ?, event_status = ?, event_edit_date = ?, directorate_id = ?, user_id_edit = ? where id = ?', 'ississsisiii', array(&$event_entity, &$event_entity_name, &$time, &$hall_id, &$event_place, &$subject, &$event_date, &$event_status, &$event_edit_date, &$directorate_id, &$user_id_edit, &$id));
    }
    ...
    function get_subject_characters($event_id) {
        return $this->get_data("SELECT CHAR_LENGTH(`subject`) AS 'character_length' "
                        . "FROM events WHERE `id` = ?", 'i', array(&$event_id));
    }
